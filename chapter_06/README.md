...
The complete process is captured in the Notebook and you can see the raw code listings from the
book in the python scripts:
...
In the second case study we expand on these ideas to extract selection crtieria from resume documents
and then ask a model to evaluate how well each candidate matches the selection criteria.

The complete process is outlined in the Notebook file, and the individual scripts contain the code
listings from the published book:
